...
class DBN(nn.Module):
    ...
    def forward(self , input_data):
        ...
        for i in range(len(self.rbm_layers)):
            v = v.view((v.shape[0] , -1)).type(torch.cuda.FloatTensor)#flatten
    ...
    def reconstruct(self,input_data):
        ...
        for i in range(len(self.rbm_layers)):
            try :
                h = h.view((h.shape[0] , -1)).type(torch.cuda.FloatTensor) #flatten
            except :
                print('eccezione')
                h = torch.flatten(h)
            if self.use_gpu:
        ...
        for i in range(len(self.rbm_layers)-1,-1,-1):
            print(v.shape)
            try:
                v = v.view((v.shape[0] , -1)).type(torch.cuda.FloatTensor)
            except:
                print('eccezione')
                v = torch.flatten(v)
            if self.use_gpu:
    ...
    def train_static(self, train_data,train_labels,num_epochs=50,batch_size=10):
        ...
        for i in range(len(self.rbm_layers)):
            ...
            tensor_x = tmp.type(torch.cuda.FloatTensor) # transform to torch tensors
            tensor_y = train_labels.type(torch.cuda.FloatTensor)
            # tensor_x = Variable(tensor_x).cuda()
            # tensor_y = Variable(tensor_y).cuda()
            ...
            v = tmp.view((tmp.shape[0] , -1)).type(torch.cuda.FloatTensor)#flatten
    ...
    def train_ith(self, train_data, train_labels, num_epochs, batch_size, ith_layer):
        ...
        v = train_data.view((train_data.shape[0] , -1)).type(torch.cuda.FloatTensor)
