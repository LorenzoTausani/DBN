...
class RBM(nn.Module):
    ...
    def __init__(self,
        ...
        if not self.xavier_init:
            ...
            self.W = -self.xavier_value + torch.FloatTensor(torch.rand(self.visible_units, self.hidden_units)).to(DEVICE) * (2 * self.xavier_value)
    ...
    def contrastive_divergence(self, input_data ,training = True,
                                n_gibbs_sampling_steps=1 , lr = 0.001, momentum=0.5):
        # positive phase
        if self.use_gpu:
            input_data.cuda()
        ...
        if(training):
            ...
            #Add annealing 
            #   ((posprods_GPU - negprods_GPU) / numcases - weightcost_GPU * vishid_GPU) * epsilonw_GPU
            self.W += self.W*momentum + (lr * (grad_update - 0.0002*self.W))
            self.v_bias += self.v_bias*momentum + (lr * v_bias_update)
            self.h_bias += self.h_bias*momentum + (lr * h_bias_update)
    ...
    def step(self,input_data,epoch,num_epochs):
        ...
        return self.contrastive_divergence(input_data , True, n_gibbs_sampling_steps, lr, momentum=momentum);
    ...
    def train(self,train_dataloader , num_epochs = 50,batch_size=16):
        ...
        for epoch in range(1 , num_epochs+1):
            ...
            cost_ = torch.cuda.FloatTensor(n_batches , 1)
            grad_ = torch.cuda.FloatTensor(n_batches , 1)
